...
test('POST /airports/:id/manual-jobs rejects unlisted airports', async () => {
  const app = express();
  app.use(express.json());
  app.use(
    createAdminRoutes({
  ...
  try {
    ...
    const response = await fetch(`http://127.0.0.1:${port}/airports/1/manual-jobs`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ kind: 'performance', date: '2026-03-30' }),
    });
    assert.equal(response.status, 409);
    const data = (await response.json()) as { code: string; message: string };
    assert.equal(data.code, 'AIRPORT_UNLISTED_MANUAL_JOB_DISABLED');
    assert.match(data.message, /已下架机场/);
  } finally {
    await new Promise<void>((resolve, reject) => server.close((error) => (error ? reject(error) : resolve())));
  }
});
      ...
      marketingSettingsService: {
        ...
        updateAdminSettings: async (input, updatedBy) => {
          ...
          return {
            ...
            home_rotation_interval_minutes: input.home_rotation_interval_minutes,
  ...
  try {
    ...
    const response = await fetch(`http://127.0.0.1:${port}/marketing/settings`, {
      ...
      body: JSON.stringify({
        ...
        admin_telegram_username: '@gaterank_admin',
        home_rotation_interval_minutes: 30,
        home_section_limits: {
          today_pick: 6,
          most_stable: 4,
          best_value: 5,
