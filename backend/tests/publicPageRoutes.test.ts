  ...
  try {
    ...
    for (const [path, h1Pattern, titlePattern] of checks) {
      ...
      if (path === '/') {
        ...
        assert.match(html, /\.home-v3-deal-actions\s*\{[^}]*margin-top:\s*auto;[^}]*border-top:\s*1px solid #f5f5f5;[^}]*padding-top:\s*6px;/);
        assert.match(html, /\.home-v3-sidebar > \.home-v3-explore\s*\{[^}]*background:\s*#1e1b4b;[^}]*color:\s*#fff;/);
        assert.match(html, /\.home-v3-deal-report\s*\{[^}]*height:\s*32px;[^}]*border-color:\s*#171717 !important;[^}]*background:\s*#171717;[^}]*color:\s*#fff;/);
        assert.match(html, /\.home-v3-deal-website\s*\{[^}]*height:\s*30px;[^}]*background:\s*#fafafa;[^}]*color:\s*#404040;/);
