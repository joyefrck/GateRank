...
test('React homepage uses shared five-slot commercial cards and plain summary scores', async () => {
  ...
  assert.match(sponsoredDealCardSource, /grid grid-cols-2 items-center gap-2/);
  ...
  assert.match(sponsoredDealCardSource, /<RouteLink[\s\S]*href=\{deal\.report_url\}[\s\S]*className="flex w-full/);
  assert.match(sponsoredDealCardSource, /className="flex w-full self-center items-center justify-center[^"]*"\s*>\s*官网/);
