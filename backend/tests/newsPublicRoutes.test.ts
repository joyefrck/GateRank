...
test('GET /news returns server-rendered HTML with aligned public header tokens', async () => {
  ...
  try {
    ...
    assert.match(html, /\.public-top-nav-links\s*\{[^}]*font-size:\s*14px;[^}]*letter-spacing:\s*0;/);
    ...
    assert.match(html, /<a class="public-top-nav-login" href="\/portal" data-client-nav="true">[\s\S]*?登录<\/a>/);
    assert.match(html, /<a class="public-top-nav-apply" href="\/apply" data-client-nav="true">/);
...
test('GET /sitemap.xml includes published news urls', async () => {
  ...
  try {
    ...
    assert.equal(urlBlocks.length, 67);
    ...
    assert.match(xml, /<loc>http:\/\/127\.0\.0\.1:\d+\/tools<\/loc>\n    <lastmod>2026-07-25T00:00:00\+08:00<\/lastmod>/);
    assert.match(xml, /<loc>http:\/\/127\.0\.0\.1:\d+\/tools\/download<\/loc>\n    <lastmod>2026-07-25T00:00:00\+08:00<\/lastmod>/);
    assert.doesNotMatch(xml, /<loc>http:\/\/127\.0\.0\.1:\d+\/download<\/loc>/);
