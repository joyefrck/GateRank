...
interface RouteState {
  kind: 'home' | 'report' | 'apply' | 'full_ranking';
  airportId?: number;
  date?: string;
  page?: number;
}
...
type NavigationKind = 'home' | 'full_ranking';
...
function buildHomeHref(date?: string): string {
  return `/${buildQuery({ date })}`;
}

function buildFullRankingHref(date?: string, page = 1): string {
...
const ConclusionCard = ({ type, title, name, website, tags, score, details, conclusion, icon, onOpen }: CardProps) => {
  const styles = {
...
function HomePage({ date }: { date?: string }) {
  ...
  usePageSeo({
    title: homepageTitle,
    description: homepageDescription,
    keywords: '机场榜,机场推荐,机场VPN,VPN,科学上网,魔法,梯子,今日推荐机场,机场测评,稳定机场,风险预警,GateRank',
...
function FullRankingPage({ date, page = 1 }: { date?: string; page?: number }) {
  ...
  usePageSeo({
    title: seoTitle,
    description: seoDescription,
    keywords: '全量榜单,机场排名,机场推荐,机场测评,机场官网,风险机场,GateRank',
...
function ReportPage({ airportId, date }: { airportId: number; date?: string }) {
  ...
  const reportTitle = data
    ? `${data.airport.name} 测评报告 | 机场榜 GateRank`
    : `机场测评报告 | 机场榜 GateRank`;
  const reportDescription = data
    ...
    : '机场榜测评报告页展示单个机场的榜单位置、评分拆解、关键指标与 30 天趋势。';
  ...
  usePageSeo({
    title: reportTitle,
    description: reportDescription,
    keywords: '机场测评报告,机场评分,机场趋势,机场榜,GateRank',
