...
性能和网络覆盖任务均最多领取执行 **3 次（首次 + 2 次重试）**。每次领取计入 `attempts`；只有租约过期才会重新排队。性能租约为 15 分钟，网络覆盖租约为 1 小时，次数限制不代表固定的总完成时限。

第三次租约仍有效时允许正常上传；租约过期后，下一次该探针领取任务会将其标记为 `expired`，保留任务和尝试次数，不再自动重试。历史上已超限的排队任务也会进入此终态，已完成任务不受影响。超限不会生成虚假采集结果或分数，缺失结果仍需按实际采集情况处理。

排查时结合 `status=expired`、`attempts>=3` 和探针 `journalctl` 日志判断耗尽原因。修复探针后若需重测，应显式派发新任务，避免清零旧任务次数。
